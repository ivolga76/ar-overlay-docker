...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${data.season.name} — AR Overlay`,
    description: data.season.description || `Турнирный сезон «${data.season.name}»`,
  };
}
...
export default async function SeasonPage({ params }: Props) {
  ...
  return (
    <main className="flex-1">
      <PageHeader
        title={season.name}
        subtitle={season.description || undefined}
        backHref="/"
        ...
        <DarkPanel className="mb-8">
          <div className="flex flex-wrap justify-center gap-8 py-4">
            <div className="text-center">
              <span className="mono-stat text-2xl text-accent-cyan font-bold">
            ...
            <div className="text-center">
              <span className="mono-stat text-2xl text-accent-gold font-bold">
            ...
            <div className="text-center">
              <span className={`mono-stat text-2xl font-bold ${season.status === 'active' ? 'text-accent-green' : 'text-text-muted'}`}>
