...
#### Виджеты оверлея
- **Название турнира** — кастомизируемое название (`Битва за Респект`)
- **Раунд** — текущий раунд и общее количество («Раунд X из Y»)
- **Счёт** — имя игрока/команды и очки (с анимацией при изменении)
- **Задачи** — сетка задач раунда с прогрессом и стоимостью
- **Таймер** — SVG-кольцо с обратным отсчётом и звуковым сигналом
- **Предыдущий игрок** — имя и очки прошлого участника
- **Турнирная таблица** — standings всех участников, сортировка по очкам
- **Усложнения** — список активных усложнений раунда с нумерацией
